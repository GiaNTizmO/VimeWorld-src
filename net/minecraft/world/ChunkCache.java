...
public class ChunkCache implements IBlockAccess
{
    ...
    public ChunkCache(World worldIn, BlockPos posFromIn, BlockPos posToIn, int subIn)
    {
        ...
        this.chunkX = posFromIn.n() - subIn >> 4;
        this.chunkZ = posFromIn.p() - subIn >> 4;
        int i = posToIn.n() + subIn >> 4;
        int j = posToIn.p() + subIn >> 4;
        ...
        for (int i1 = posFromIn.n() >> 4; i1 <= posToIn.n() >> 4; ++i1)
        {
            for (int j1 = posFromIn.p() >> 4; j1 <= posToIn.p() >> 4; ++j1)
            {
                Chunk chunk = this.chunkArray[i1 - this.chunkX][j1 - this.chunkZ];

                if (chunk != null && !chunk.getAreLevelsEmpty(posFromIn.o(), posToIn.o()))
                {
    ...
    {
        int i = (pos.n() >> 4) - this.chunkX;
        int j = (pos.p() >> 4) - this.chunkZ;
