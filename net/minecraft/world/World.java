...
public abstract class World implements IBlockAccess
{
    ...
    public Block getGroundAboveSeaLevel(BlockPos pos)
    {
        ...
        for (blockpos = new BlockPos(pos.n(), this.func_181545_F(), pos.p()); !this.isAirBlock(blockpos.up()); blockpos = blockpos.up())
        {
            ;
        }
     ...
     */
    private static boolean isValid(BlockPos p_175701_0_)
    {
        return p_175701_0_.n() >= -30000000 && p_175701_0_.p() >= -30000000 && p_175701_0_.n() < 30000000 && p_175701_0_.p() < 30000000 && p_175701_0_.o() >= 0 && p_175701_0_.o() < 256;
    }
    ...
    {
        return isValid(pos) && this.isChunkLoaded(pos.n() >> 4, pos.p() >> 4, allowEmpty);
    }
    ...
    {
        return this.isAreaLoaded(center.n() - radius, center.o() - radius, center.p() - radius, center.n() + radius, center.o() + radius, center.p() + radius, allowEmpty);
    }
    ...
    {
        return this.isAreaLoaded(from.n(), from.o(), from.p(), to.n(), to.o(), to.p(), allowEmpty);
    }
    ...
    {
        return this.getChunkFromChunkCoords(pos.n() >> 4, pos.p() >> 4);
    }
    ...
    {
        this.markBlockRangeForRenderUpdate(rangeMin.n(), rangeMin.o(), rangeMin.p(), rangeMax.n(), rangeMax.o(), rangeMax.p());
    }
    ...
    {
        if (pos.o() >= this.func_181545_F())
        {
            return this.canSeeSky(pos);
        }
        else
        {
            BlockPos blockpos = new BlockPos(pos.n(), this.func_181545_F(), pos.p());
            ...
            {
                for (blockpos = blockpos.down(); blockpos.o() > pos.o(); blockpos = blockpos.down())
                {
    ...
    public int getLight(BlockPos pos)
    {
        ...
        else
        {
            if (pos.o() >= 256)
            {
                pos = new BlockPos(pos.n(), 255, pos.p());
            }
    ...
    {
        if (pos.n() >= -30000000 && pos.p() >= -30000000 && pos.n() < 30000000 && pos.p() < 30000000)
        {
    ...
    public BlockPos getHeight(BlockPos pos)
    {
        ...
        if (pos.n() >= -30000000 && pos.p() >= -30000000 && pos.n() < 30000000 && pos.p() < 30000000)
        {
            if (this.isChunkLoaded(pos.n() >> 4, pos.p() >> 4, true))
            {
                i = this.getChunkFromChunkCoords(pos.n() >> 4, pos.p() >> 4).getHeightValue(pos.n() & 15, pos.p() & 15);
            }
            else
            {
                i = 0;
        ...
        return new BlockPos(pos.n(), i, pos.p());
    }
    ...
    public int getLightFromNeighborsFor(EnumSkyBlock type, BlockPos pos)
    {
        ...
        else
        {
            if (pos.o() < 0)
            {
                pos = new BlockPos(pos.n(), 0, pos.p());
            }
    ...
    public int getLightFor(EnumSkyBlock type, BlockPos pos)
    {
        ...
        return pos.o() < 256 && this.chunkProvider.chunkExists(pos.n() >> 4, pos.p() >> 4) ? this.chunkProvider.provideChunk(pos.n() >> 4, pos.p() >> 4).getLightFor(type, pos) : type.defaultLightValue;
    }

    public void setLightFor(EnumSkyBlock type, BlockPos pos, int lightValue)
    {
        if (isValid(pos) && this.isBlockLoaded(pos))
        {
            Chunk chunk = this.getChunkFromBlockCoords(pos);
    ...
    public List<AxisAlignedBB> getCollidingBoundingBoxes(Entity entityIn, AxisAlignedBB bb)
    {
        ...
        for (int k1 = i; k1 < j; ++k1)
        {
            ...
            {
                if (this.isBlockLoaded(mutableblockpos.func_181079_c(k1, 64, l1)))
                {
                    for (int i2 = k - 1; i2 < l; ++i2)
                    {
                        ...
                        {
                            iblockstate1 = this.getBlockState(mutableblockpos);
                        }

                        iblockstate1.getBlock().addCollisionBoxesToList(this, mutableblockpos, iblockstate1, bb, list, entityIn);
                    }
                }
            }
        }
        ...
        Texteria.instance.world.boxes.getCollisionBoxes(entityIn, bb, list);
        return list;
    }
            ...
            for (int l1 = i1; l1 < j1; ++l1)
            {
                if (this.isBlockLoaded(mutableblockpos.func_181079_c(k1, 64, l1)))
                {
                    for (int i2 = k - 1; i2 < l; ++i2)
                    {
                        ...
                        {
                            iblockstate = this.getBlockState(mutableblockpos);
                        }
                        else
                        {
                        ...
                        iblockstate.getBlock().addCollisionBoxesToList(this, mutableblockpos, iblockstate, bb, list, (Entity)null);
                    }
                }
            }
        }
    ...
    public BlockPos getTopSolidOrLiquidBlock(BlockPos pos)
    {
        ...
        for (blockpos = new BlockPos(pos.n(), chunk.getTopFilledSegment() + 16, pos.p()); blockpos.o() >= 0; blockpos = blockpos1)
        {
    ...
    public void updateEntities()
    {
        ...
        for (int i = 0; i < this.weatherEffects.size(); ++i)
        {
            ...
            {
                CrashReport crashreport = CrashReport.makeCrashReport(throwable21, "Ticking entity");
        ...
        this.theProfiler.endSection();
        this.theProfiler.endSection();
        Texteria.instance.world.boxes.tick();
    }
    ...
    public boolean checkBlockCollision(AxisAlignedBB bb)
    {
        ...
        for (int k1 = i; k1 <= j; ++k1)
        {
            for (int l1 = k; l1 <= l; ++l1)
            {
                ...
                {
                    Block block = this.getBlockState(mutableblockpos.func_181079_c(k1, l1, i2)).getBlock();
                ...
                for (int l1 = k; l1 < l; ++l1)
                {
                    for (int i2 = i1; i2 < j1; ++i2)
                    {
                        ...
                        if (block.getMaterial() == materialIn)
                        {
                            ...
                            {
                                flag = true;
                                vec3 = block.modifyAcceleration(this, mutableblockpos, entityIn, vec3);
                            }
                        }
                    }
                }
    ...
    public boolean isMaterialInBB(AxisAlignedBB bb, Material materialIn)
    {
        ...
        for (int k1 = i; k1 < j; ++k1)
        {
            for (int l1 = k; l1 < l; ++l1)
            {
                for (int i2 = i1; i2 < j1; ++i2)
                {
                    if (this.getBlockState(mutableblockpos.func_181079_c(k1, l1, i2)).getBlock().getMaterial() == materialIn)
                    {
                        return true;
                    }
                }
    ...
    public boolean isAABBInMaterial(AxisAlignedBB bb, Material materialIn)
    {
        ...
        for (int k1 = i; k1 < j; ++k1)
        {
            for (int l1 = k; l1 < l; ++l1)
            {
                for (int i2 = i1; i2 < j1; ++i2)
                {
                    IBlockState iblockstate = this.getBlockState(mutableblockpos.func_181079_c(k1, l1, i2));
                    Block block = iblockstate.getBlock();
    ...
    public static boolean doesBlockHaveSolidTopSurface(IBlockAccess blockAccess, BlockPos pos)
    {
        ...
        Block block = iblockstate.getBlock();
        return block.getMaterial().isOpaque() && block.isFullCube() ? true : (block instanceof BlockStairs ? iblockstate.getValue(BlockStairs.HALF) == BlockStairs.EnumHalf.TOP : (block instanceof BlockSlab ? iblockstate.getValue(BlockSlab.HALF) == EnumBlockHalf.TOP : (block instanceof BlockHopper ? true : (block instanceof BlockSnow ? ((Integer)iblockstate.getValue(BlockSnow.LAYERS)).intValue() == 7 : false))));
    }
    ...
    public boolean canBlockFreeze(BlockPos pos, boolean noWaterAdj)
    {
        ...
        {
            if (pos.o() >= 0 && pos.o() < 256 && this.getLightFor(EnumSkyBlock.BLOCK, pos) < 10)
            {
                IBlockState iblockstate = this.getBlockState(pos);
                Block block = iblockstate.getBlock();
    ...
    private int getRawLight(BlockPos pos, EnumSkyBlock lightType)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    mutableblockpos.func_181079_c(pos.n() + enumfacing.getFrontOffsetX(), pos.o() + enumfacing.getFrontOffsetY(), pos.p() + enumfacing.getFrontOffsetZ());
                    int k = this.getLightFor(lightType, mutableblockpos) - j;
    ...
    {
        if (!TexteriaOptions.worldLightUpdates)
        {
            return false;
        }
        ...
        else
        {
            ...
            else if (l < k)
            {
                ...
                while (i < j)
                {
                    ...
                    int l2 = l1 >> 18 & 15;
                    mutableblockpos.func_181079_c(i2, j2, k2);
                    int i3 = this.getLightFor(lightType, mutableblockpos);

                    if (i3 == l2)
                    {
                        this.setLightFor(lightType, mutableblockpos, 0);
                        ...
                        if (l2 > 0)
                        {
                            ...
                            if (j3 + k3 + l3 < 17)
                            {
                                ...
                                for (EnumFacing enumfacing : EnumFacing.VALUES)
                                {
                                    ...
                                    mutableblockpos1.func_181079_c(i4, j4, k4);
                                    int l4 = Math.max(1, this.getBlockState(mutableblockpos1).getBlock().getLightOpacity());
                                    i3 = this.getLightFor(lightType, mutableblockpos1);
            ...
            while (i < j)
            {
                ...
                BlockPos blockpos = mutableblockpos.func_181079_c(j5, k5, l5);
                int i6 = this.getLightFor(lightType, blockpos);
                int j6 = this.getRawLight(blockpos, lightType);

                if (j6 != i6)
                {
                    this.setLightFor(lightType, blockpos, j6);
                    ...
                    if (j6 > i6)
                    {
                        ...
                        if (k6 + l6 + i7 < 17 && flag)
                        {
                            if (this.getLightFor(lightType, blockpos.west()) < j6)
                            {
                                this.lightUpdateBlockList[j++] = j5 - 1 - i1 + 32 + (k5 - j1 + 32 << 6) + (l5 - k1 + 32 << 12);
                            }

                            if (this.getLightFor(lightType, blockpos.east()) < j6)
                            {
                                this.lightUpdateBlockList[j++] = j5 + 1 - i1 + 32 + (k5 - j1 + 32 << 6) + (l5 - k1 + 32 << 12);
                            }

                            if (this.getLightFor(lightType, blockpos.down()) < j6)
                            {
                                this.lightUpdateBlockList[j++] = j5 - i1 + 32 + (k5 - 1 - j1 + 32 << 6) + (l5 - k1 + 32 << 12);
                            }

                            if (this.getLightFor(lightType, blockpos.up()) < j6)
                            {
                                this.lightUpdateBlockList[j++] = j5 - i1 + 32 + (k5 + 1 - j1 + 32 << 6) + (l5 - k1 + 32 << 12);
                            }

                            if (this.getLightFor(lightType, blockpos.north()) < j6)
                            {
                                this.lightUpdateBlockList[j++] = j5 - i1 + 32 + (k5 - j1 + 32 << 6) + (l5 - 1 - k1 + 32 << 12);
                            }

                            if (this.getLightFor(lightType, blockpos.south()) < j6)
                            {
                                this.lightUpdateBlockList[j++] = j5 - i1 + 32 + (k5 - j1 + 32 << 6) + (l5 + 1 - k1 + 32 << 12);
                            }
                        }
    ...
    public boolean canLightningStrike(BlockPos strikePosition)
    {
        ...
        else if (this.getPrecipitationHeight(strikePosition).o() > strikePosition.o())
        {
            return false;
        }
        else
    ...
    public void updateComparatorOutputLevel(BlockPos pos, Block blockIn)
    {
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            ...
            if (this.isBlockLoaded(blockpos))
            {
                ...
                if (Blocks.unpowered_comparator.e(iblockstate.getBlock()))
                {
    ...
    public boolean isSpawnChunk(int x, int z)
    {
        ...
        int i = x * 16 + 8 - blockpos.n();
        int j = z * 16 + 8 - blockpos.p();
        int k = 128;
