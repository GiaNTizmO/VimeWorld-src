...
public class Chunk
{
    ...
    public Chunk(World worldIn, int x, int z)
    {
        ...
        this.xPosition = x;
        this.zPosition = z;
        this.hashedLoc = ChunkCoordIntPair.chunkXZ2Int(x, z);
    ...
    {
        return this.getHeightValue(pos.n() & 15, pos.p() & 15);
    }
            ...
            for (int i = 0; i < 16; ++i)
            {
                for (int j = 0; j < 16; ++j)
                {
                    if (this.updateSkylightColumns[i + j * 16])
                    {
                        ...
                        {
                            j1 = Math.min(j1, this.worldObj.getChunksLowestHorizon(l + ((EnumFacing)object).getFrontOffsetX(), i1 + ((EnumFacing)object).getFrontOffsetZ()));
                        }

                        this.checkSkylightNeighborHeight(l, i1, j1);

                        for (Object object1 : EnumFacing.Plane.HORIZONTAL)
                        {
                            this.checkSkylightNeighborHeight(l + ((EnumFacing)object1).getFrontOffsetX(), i1 + ((EnumFacing)object1).getFrontOffsetZ(), k);
                        }
    ...
    {
        int i = this.worldObj.getHeight(new BlockPos(x, 0, z)).o();
    ...
    private void relightBlock(int x, int y, int z)
    {
        ...
        if (j != i)
        {
            ...
            if (!this.worldObj.provider.getHasNoSky())
            {
                for (Object object : EnumFacing.Plane.HORIZONTAL)
                {
                    this.updateSkylightNeighborHeight(k + ((EnumFacing)object).getFrontOffsetX(), l + ((EnumFacing)object).getFrontOffsetZ(), j2, k2);
                }
    ...
    public Block getBlock(final BlockPos pos)
    {
        try
        {
            return this.getBlock0(pos.n() & 15, pos.o(), pos.p() & 15);
        }
        catch (ReportedException reportedexception)
        {
            CrashReportCategory crashreportcategory = reportedexception.getCrashReport().makeCategory("Block being got");
    ...
    public IBlockState getBlockState(final BlockPos pos)
    {
        if (this.worldObj.getWorldType() == WorldType.DEBUG_WORLD)
        {
            ...
            if (pos.o() == 70)
            {
                iblockstate = ChunkProviderDebug.func_177461_b(pos.n(), pos.p());
            }
        ...
        else
        {
            try
            {
                if (pos.o() >= 0 && pos.o() >> 4 < this.storageArrays.length)
                {
                    ExtendedBlockStorage extendedblockstorage = this.storageArrays[pos.o() >> 4];
    ...
    {
        return this.getBlockMetadata(pos.n() & 15, pos.o(), pos.p() & 15);
    }
    ...
    {
        if (TexteriaOptions.worldLightForced != -1)
        {
            return TexteriaOptions.worldLightForced;
        }
        else
        {
            ExtendedBlockStorage extendedblockstorage = this.storageArrays[pos.o() >> 4];
            return extendedblockstorage == null ? (pos.o() >= this.heightMap[(pos.n() & 15) << 4 | pos.p() & 15] ? p_177413_1_.defaultLightValue : 0) : (p_177413_1_ == EnumSkyBlock.SKY ? (this.worldObj.provider.getHasNoSky() ? 0 : extendedblockstorage.getExtSkylightValue(pos.n() & 15, pos.o() & 15, pos.p() & 15)) : extendedblockstorage.getExtBlocklightValue(pos.n() & 15, pos.o() & 15, pos.p() & 15));
        }
    }
    ...
    public BlockPos getPrecipitationHeight(BlockPos pos)
    {
        ...
        int j = pos.p() & 15;
        int k = i | j << 4;
        BlockPos blockpos = new BlockPos(pos.n(), this.precipitationHeightMap[k], pos.p());

        if (blockpos.o() == -999)
        {
            int l = this.getTopFilledSegment() + 15;
            blockpos = new BlockPos(pos.n(), l, pos.p());
            int i1 = -1;
        ...
        return new BlockPos(pos.n(), this.precipitationHeightMap[k], pos.p());
    }
    ...
    public void enqueueRelightChecks()
    {
        ...
        for (int k = 0; k < 8; ++k)
        {
            ...
            int l = this.queuedLightChecks % 16;
            int i1 = this.queuedLightChecks / 16 % 16;
            int j1 = this.queuedLightChecks / 256;
            ++this.queuedLightChecks;

            for (int k1 = 0; k1 < 16; ++k1)
            {
                BlockPos blockpos = mutableblockpos.func_181079_c(i + i1, (l << 4) + k1, j + j1);
                boolean flag = k1 == 0 || k1 == 15 || i1 == 0 || i1 == 15 || j1 == 0 || j1 == 15;

                if (this.storageArrays[l] == null && flag || this.storageArrays[l] != null && this.storageArrays[l].getBlockByExtId(i1, k1, j1).getMaterial() == Material.air)
                {
                    for (EnumFacing enumfacing : EnumFacing.VALUES)
                    {
                        BlockPos blockpos1 = mutableblockpos1.func_181079_c(mutableblockpos.getX() + enumfacing.getFrontOffsetX(), mutableblockpos.getY() + enumfacing.getFrontOffsetY(), mutableblockpos.getZ());

                        if (this.worldObj.getBlockState(blockpos1).getBlock().getLightValue() > 0)
                        {
                            this.worldObj.checkLight(blockpos1);
                        }
                    }

                    this.worldObj.checkLight(blockpos);
                }
            }
        }
    }
        ...
        if (!this.worldObj.provider.getHasNoSky())
        {
            if (this.worldObj.isAreaLoaded(blockpos.add(-1, 0, -1), blockpos.add(16, this.worldObj.func_181545_F(), 16)))
            {
                ...
                if (this.isLightPopulated)
                {
                    for (Object object : EnumFacing.Plane.HORIZONTAL)
                    {
                        int k = ((EnumFacing)object).getAxisDirection() == EnumFacing.AxisDirection.POSITIVE ? 16 : 1;
                        this.worldObj.getChunkFromBlockCoords(blockpos.offset((EnumFacing)object, k)).func_180700_a(((EnumFacing)object).getOpposite());
                    }
    ...
    private boolean func_150811_f(int x, int z)
    {
        ...
        boolean flag = false;
        boolean flag1 = false;
        MutableBlockPos mutableblockpos = new MutableBlockPos((this.xPosition << 4) + x, 0, (this.zPosition << 4) + z);

        for (int j = i + 16 - 1; j > this.worldObj.func_181545_F() || j > 0 && !flag1; --j)
        {
            mutableblockpos.func_181079_c(mutableblockpos.getX(), j, mutableblockpos.getZ());
            int k = this.getBlockLightOpacity(mutableblockpos);

            if (k == 255 && mutableblockpos.getY() < this.worldObj.func_181545_F())
            {
                flag1 = true;
            }
            ...
            else if (flag && k == 0 && !this.worldObj.checkLight(mutableblockpos))
            {
                return false;
            }
        }

        for (int l = mutableblockpos.getY(); l > 0; --l)
        {
            mutableblockpos.func_181079_c(mutableblockpos.getX(), l, mutableblockpos.getZ());
