...
public class EntityAIControlledByPlayer extends EntityAIBase
{
    ...
    public void updateTask()
    {
        ...
        {
            f2 = this.thisEntity.worldObj.getBlockState(new BlockPos(MathHelper.floor_float((float)i), MathHelper.floor_float((float)j) - 1, MathHelper.floor_float((float)k))).getBlock().slipperiness.get() * 0.91F;
        }

        float f3 = 0.16277136F / (f2 * f2 * f2);
        float f4 = MathHelper.sin(entitycreature.y * (float)Math.PI / 180.0F);
        float f5 = MathHelper.cos(entitycreature.y * (float)Math.PI / 180.0F);
