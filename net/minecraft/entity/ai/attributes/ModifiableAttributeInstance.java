...
public class ModifiableAttributeInstance implements IAttributeInstance
{
    ...
    public static class Protected extends ModifiableAttributeInstance
    {
        private ObfValue.ODouble obfBase;
        private ObfValue.ODouble obfCached;

        public Protected(BaseAttributeMap p_i6_1_, IAttribute p_i6_2_)
        {
            super(p_i6_1_, p_i6_2_);
            this.obfBase = new ObfValue.ODouble(this.baseValue);
            this.obfCached = new ObfValue.ODouble(this.cachedValue);
        }

        public double getBaseValue()
        {
            double d0 = super.getBaseValue();

            if (!this.obfBase.equals(d0, 1.0E-5D))
            {
                throw new RuntimeException("Broken attribute base value");
            }
            else
            {
                return d0;
        ...
        public double getAttributeValue()
        {
            double d0 = super.getAttributeValue();

            if (!this.obfCached.equals(d0, 1.0E-5D))
            {
                throw new RuntimeException("Broken attribute value");
            }
            else
            {
                return d0;
        ...
        public void setBaseValue(double baseValue)
        {
            ...
            this.obfBase.set(baseValue);
        }

        protected double computeValue()
        {
            double d0 = super.computeValue();
            this.obfCached.set(d0);
            return d0;
        }
    }
}
