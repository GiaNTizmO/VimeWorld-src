...
public abstract class BlockLiquid extends Block
{
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL)
        {
            ...
            if (j < 0)
            {
                if (!worldIn.getBlockState(blockpos).getBlock().getMaterial().blocksMovement())
                {
                    ...
                    {
                        int k = j - (i - 8);
                        vec3 = vec3.addVector((double)((blockpos.n() - pos.n()) * k), (double)((blockpos.o() - pos.o()) * k), (double)((blockpos.p() - pos.p()) * k));
                    }
                }
            }
            else if (j >= 0)
            {
                int l = j - i;
                vec3 = vec3.addVector((double)((blockpos.n() - pos.n()) * l), (double)((blockpos.o() - pos.o()) * l), (double)((blockpos.p() - pos.p()) * l));
            }
        }
    ...
    {
        double d0 = (double)pos.n();
        double d1 = (double)pos.o();
        double d2 = (double)pos.p();
    ...
    {
        Vec3 vec3 = getFlowingBlock(materialIn).h(worldIn, pos);
