    ...
    public void processPacket(INetHandlerPlayClient handler)
    {
        if (Minecraft.getMinecraft().isCallingFromMinecraftThread())
        {
            if (Brainstorm.isChunksBroken())
            {
                try
                {
                    NativeGameUtils.fixPacketChunk(this);
                }
                catch (UnsatisfiedLinkError var3)
                {
                    Brainstorm.fallback().fixPacketChunk(this);
                }
            }

            if (Brainstorm.debug)
            {
                Brainstorm.log.info("[Brainstorm] Chunk " + this.chunkX + " " + this.chunkZ + " was " + (Brainstorm.isChunksBroken() ? "obfuscated" : "not obfuscated"));
            }
        }
