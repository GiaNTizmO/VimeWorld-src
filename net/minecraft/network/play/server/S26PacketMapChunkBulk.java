...
import net.xtrafrancyz.covered.Brainstorm;
import ru.vimeworld.brainstorm.mc18.NativeGameUtils;
...
public class S26PacketMapChunkBulk implements Packet<INetHandlerPlayClient>
{
    ...
    public void processPacket(INetHandlerPlayClient handler)
    {
        ...
        {
            if (Brainstorm.isChunksBroken())
            {
                try
                {
                    NativeGameUtils.fixPacketChunkBulk(this);
                }
                catch (UnsatisfiedLinkError var3)
                {
                    Brainstorm.fallback().fixPacketChunkBulk(this);
                }
            }

            if (Brainstorm.debug)
            {
                Brainstorm.log.info("[Brainstorm] " + this.xPositions.length + " chunks was " + (Brainstorm.isChunksBroken() ? "obfuscated" : "not obfuscated"));
            }
        }
