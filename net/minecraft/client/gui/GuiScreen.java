...
public abstract class GuiScreen extends Gui implements GuiYesNoCallback
{
    ...
    protected void drawHoveringText(List<String> textLines, int x, int y)
    {
        if (!textLines.isEmpty())
        {
            ...
            int k = -267386864;
            this.drawGradientRect(k1 - 3, l1 - 4, k1 + i + 3, l1 - 3, k, k);
            this.drawGradientRect(k1 - 3, l1 + i2 + 3, k1 + i + 3, l1 + i2 + 4, k, k);
            this.drawGradientRect(k1 - 3, l1 - 3, k1 + i + 3, l1 + i2 + 3, k, k);
            this.drawGradientRect(k1 - 4, l1 - 3, k1 - 3, l1 + i2 + 3, k, k);
            this.drawGradientRect(k1 + i + 3, l1 - 3, k1 + i + 4, l1 + i2 + 3, k, k);
            int l = 1347420415;
            int i1 = (l & 16711422) >> 1 | l & -16777216;
            this.drawGradientRect(k1 - 3, l1 - 3 + 1, k1 - 3 + 1, l1 + i2 + 3 - 1, l, i1);
            this.drawGradientRect(k1 + i + 2, l1 - 3 + 1, k1 + i + 3, l1 + i2 + 3 - 1, l, i1);
            this.drawGradientRect(k1 - 3, l1 - 3, k1 + i + 3, l1 - 3 + 1, l, l);
            this.drawGradientRect(k1 - 3, l1 + i2 + 2, k1 + i + 3, l1 + i2 + 3, i1, i1);

            for (int j1 = 0; j1 < textLines.size(); ++j1)
            {
                String s1 = (String)textLines.get(j1);
                this.fontRendererObj.drawStringWithShadow(s1, (float)k1, (float)l1, -1);
    ...
    protected void handleComponentHover(IChatComponent p_175272_1_, int p_175272_2_, int p_175272_3_)
    {
        if (p_175272_1_ != null && p_175272_1_.getChatStyle().getChatHoverEvent() != null)
        {
            ...
            else if (hoverevent.getAction() == Action.SHOW_ACHIEVEMENT)
            {
    ...
    protected boolean handleComponentClick(IChatComponent p_175276_1_)
    {
        ...
        else
        {
            ...
            {
                if (clickevent.getAction() == net.minecraft.event.ClickEvent.Action.OPEN_URL)
                {
                ...
                else if (clickevent.getAction() == net.minecraft.event.ClickEvent.Action.OPEN_FILE)
                {
                    URI uri1 = (new File(clickevent.getValue())).toURI();
                    this.openWebLink(uri1);
                }
                else if (clickevent.getAction() == net.minecraft.event.ClickEvent.Action.SUGGEST_COMMAND)
                {
                    this.setText(clickevent.getValue(), true);
                }
                else if (clickevent.getAction() == net.minecraft.event.ClickEvent.Action.RUN_COMMAND)
                {
                    this.sendChatMessage(clickevent.getValue(), false);
                }
                else if (clickevent.getAction() == net.minecraft.event.ClickEvent.Action.TWITCH_USER_INFO)
                {
    ...
    public void handleMouseInput() throws IOException
    {
        ...
        Texteria.instance.handleMouseEvent();
    ...
    private void openWebLink(URI p_175282_1_)
    {
        ...
        if (!"http".equals(p_175282_1_.getScheme()) && !"https".equals(p_175282_1_.getScheme()))
        {
            if ("file".equals(p_175282_1_.getScheme()) && (new File(p_175282_1_.getPath())).exists())
            {
                for (String s : Arrays.asList(new String[] {".png", ".jpg", ".txt"}))
                {
                    if (p_175282_1_.getPath().endsWith(s))
                    {
                        flag = true;
                    }
                }
        ...
        if (flag)
        {
            if (Platform.isWindows())
            {
                try
                {
                    Runtime.getRuntime().exec("rundll32 url.dll,FileProtocolHandler " + p_175282_1_.toString());
                    return;
                }
                catch (IOException ioexception)
                {
                    LOGGER.error((String)"Couldn\'t open link with rundll32", (Throwable)ioexception);
                }
            }
