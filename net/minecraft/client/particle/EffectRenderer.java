...
public class EffectRenderer
{
    ...
    {
        if (effect != null && (!(effect instanceof SparkFX) || Config.isFireworkParticles()))
        {
    ...
    public void updateEffects()
    {
        ...
        for (Object object : this.particleEmitters)
        {
            EntityParticleEmitter entityparticleemitter = (EntityParticleEmitter)object;
            entityparticleemitter.onUpdate();

            if (entityparticleemitter.I)
            {
        ...
        Texteria.instance.world.tickParticles();
    }
    ...
    public void renderParticles(Entity entityIn, float partialTicks)
    {
        ...
        Texteria.instance.world.renderParticles(partialTicks);
        GlStateManager.depthMask(true);
        ...
        if (flag)
        {
            ...
            for (int i = 0; i < b0; ++i)
            {
                for (int j = 0; j < b0; ++j)
                {
                    for (int k = 0; k < b0; ++k)
                    {
                        double d0 = (double)pos.n() + ((double)i + 0.5D) / (double)b0;
                        double d1 = (double)pos.o() + ((double)j + 0.5D) / (double)b0;
                        double d2 = (double)pos.p() + ((double)k + 0.5D) / (double)b0;
                        this.addEffect((new EntityDiggingFX(this.worldObj, d0, d1, d2, d0 - (double)pos.n() - 0.5D, d1 - (double)pos.o() - 0.5D, d2 - (double)pos.p() - 0.5D, state)).func_174846_a(pos));
                    }
                }
            }
        }
    ...
    public void addBlockHitEffects(BlockPos pos, EnumFacing side)
    {
        ...
        if (block.getRenderType() != -1)
        {
            ...
            this.addEffect((new EntityDiggingFX(this.worldObj, d0, d1, d2, 0.0D, 0.0D, 0.0D, iblockstate)).func_174846_a(pos).a(0.2F).multipleParticleScaleBy(0.6F));
        }
    }
    ...
    private boolean reuseBarrierParticle(EntityFX p_reuseBarrierParticle_1_, List<EntityFX> p_reuseBarrierParticle_2_)
    {
        ...
        {
            if (entityfx instanceof Barrier && p_reuseBarrierParticle_1_.s == entityfx.s && p_reuseBarrierParticle_1_.t == entityfx.t && p_reuseBarrierParticle_1_.u == entityfx.u)
            {
