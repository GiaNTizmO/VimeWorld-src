...
public class Beam extends WorldGroup
{
    ...
    public void render(long time)
    {
        if (this.preRender(time))
        {
            ...
            this.renderBeam(this.setAlpha(this.color.renderValue(time), 16), false);
            GlStateManager.popMatrix();
            GlStateManager.rotate((float)(time / 20L % 360L), 0.0F, 1.0F, 0.0F);
            GlStateManager.translate(-5.0F, 0.0F, 5.0F);
            GlStateManager.pushMatrix();
            this.renderBeam(this.color.renderValue(time), true);
            GlStateManager.popMatrix();
            GlStateManager.pushMatrix();
            GlStateManager.translate(2.5F, 0.0F, -2.5F);
            GlStateManager.scale(0.5F, 1.0F, 0.5F);
            this.renderBeam(this.alterBrightness(this.color.renderValue(time), 190), true);
            GlStateManager.popMatrix();
            GlStateManager.popMatrix();
        }
    }
    ...
    private void renderBeam(int color, boolean corners)
    {
        ...
        for (int k = 0; k < 4; ++k)
        {
            if (corners)
            {
                ...
                for (int l = 0; l < 2; ++l)
                {
                    ...
                    this.drawRect(-1.0F, (float)(-i), 1.0F, (float)(-j));
                    GlStateManager.translate(-1.0F, 0.0F, 0.0F);
            ...
            this.drawRect(-5.0F, (float)(-i), 5.0F, (float)(-j));
            GlStateManager.translate(5.0F, 0.0F, 0.0F);
            GlStateManager.rotate(90.0F, 0.0F, 1.0F, 0.0F);
        }
