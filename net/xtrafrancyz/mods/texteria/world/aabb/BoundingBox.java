    ...
    public void tick()
    {
        ...
        if (this.moveProgress > 0)
        {
            ...
            for (Entity entity : getEntitiesWithinAABB(world, this.bb, this.affectOnlyPlayers))
            {
                ...
                if (axisalignedbb2.maxX > this.bb.minX)
                {
                    treemap.put(Double.valueOf(Math.abs(axisalignedbb2.maxX - this.bb.minX)), () ->
                    {
                        moveEntity(entity, this.bb.minX - ebb.maxX, 0.0D, 0.0D);
                    });
                }

                if (axisalignedbb2.minX < this.bb.maxX)
                {
                    treemap.put(Double.valueOf(Math.abs(axisalignedbb2.minX - this.bb.maxX)), () ->
                    {
                        moveEntity(entity, this.bb.maxX - ebb.minX, 0.0D, 0.0D);
                    });
                }

                if (axisalignedbb2.maxY > this.bb.minY)
                {
                    treemap.put(Double.valueOf(Math.abs(axisalignedbb2.maxY - this.bb.minY)), () ->
                    {
                        moveEntity(entity, 0.0D, this.bb.minY - ebb.maxY, 0.0D);
                    });
                }

                if (axisalignedbb2.minY < this.bb.maxY)
                {
                    treemap.put(Double.valueOf(Math.abs(axisalignedbb2.minY - this.bb.maxY)), () ->
                    {
                        moveEntity(entity, 0.0D, this.bb.maxY - ebb.minY, 0.0D);
                    });
                }

                if (axisalignedbb2.maxZ > this.bb.minZ)
                {
                    treemap.put(Double.valueOf(Math.abs(axisalignedbb2.maxZ - this.bb.minZ)), () ->
                    {
                        moveEntity(entity, 0.0D, 0.0D, this.bb.minZ - ebb.maxZ);
                    });
                }

                if (axisalignedbb2.minZ < this.bb.maxZ)
                {
                    treemap.put(Double.valueOf(Math.abs(axisalignedbb2.minZ - this.bb.maxZ)), () ->
                    {
                        moveEntity(entity, 0.0D, 0.0D, this.bb.maxZ - ebb.minZ);
                    });
                }
