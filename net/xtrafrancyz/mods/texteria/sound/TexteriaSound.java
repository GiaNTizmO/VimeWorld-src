...
public class TexteriaSound
{
    ...
    private void realPlay(TSound sound)
    {
        ...
        if (resourcelocation.getResourceDomain().equals("texteria-sound") && resourcelocation.getResourcePath().startsWith("file:"))
        {
            File file1 = this.getLocalFile(resourcelocation);

            if (!file1.canRead())
            {
                Texteria.log.warn("Could not play sound " + file1 + " (exists=" + file1.exists() + ")");
                return;
            }
        }
