...
public class TexteriaResourceManager
{
        ...
        switch (b0)
        {
            case 0:
                ...
                for (int j = 0; j < abyte2.length / 16; ++j)
                {
                    ...
                    if (!this.resources.containsKey(s2))
                    {
    ...
    {
        TexteriaResource texteriaresource = (TexteriaResource)this.resources.get(id);
    ...
    public static String asHex(byte[] data, int offset, int length)
    {
        ...
        for (int j = 0; i < offset + length; ++i)
        {
            achar[j++] = HEX_CHARS[data[i] >>> 4 & 15];
            achar[j++] = HEX_CHARS[data[i] & 15];
        }
