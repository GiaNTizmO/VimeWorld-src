...
import net.xtrafrancyz.util.GifDecoder;
...
public class Image extends Rectangle
{
    ...
    public Image.TextureRenderHint textureRenderHint;
    private static byte[] PNG_MAGIC = new byte[] {(byte) - 119, (byte)80, (byte)78, (byte)71, (byte)13, (byte)10, (byte)26, (byte)10};

    public Image(ByteMap params)
    {
        super(params);
        this.useCache = params.getBoolean("cache", true);
        ...
        this.image = createImage(new Image.ImageDescriptor(params.getString("image"), this.textureRenderHint), params.getByteArray("idata"), this.useCache);
    ...
    public void dispose()
    {
        if (!this.useCache && this.image != null)
        {
            this.image.dispose();
        }
    ...
    public static Image.ImageInfo createImage(Image.ImageDescriptor desc, byte[] data, boolean useCache)
    {
        Image.ImageInfo image$imageinfo = useCache ? (Image.ImageInfo)inMemoryCache.get(desc) : null;

        if (image$imageinfo == null)
        {
            if (!desc.id.startsWith("http:") && !desc.id.startsWith("https:") && !desc.id.startsWith("file:") && !desc.id.startsWith("#"))
            {
                if (!desc.id.startsWith("data:"))
                {
                    if (desc.id.startsWith("item:"))
                    {
                        ItemStack itemstack = parseItemStack(desc.id.substring(5));
                    ...
                    else if (!desc.id.startsWith("head:") && !desc.id.startsWith("helm:"))
                    {
                        if (!desc.id.startsWith("btex:"))
                        {
                            Texteria.log.warn("Image id \'" + desc.id + "\' not resolved");
                            return null;
                        }

                        image$imageinfo = new Image.BlocksTextureImageInfo(desc.id.substring(5));
                    }
                    else
                    {
                        image$imageinfo = new Image.HeadImageInfo(desc.id.substring(5), desc.id.startsWith("helm:"));
                    }
                }
                else
                {
                    image$imageinfo = new Image.TexturedImageInfo(!useCache || desc.textureRenderHint != Image.TextureRenderHint.CLAMP);
                    (new Image.ByteLoaderThread(desc.id, data, (Image.TexturedImageInfo)image$imageinfo)).start();
                }
            }
            else
            {
                image$imageinfo = new Image.TexturedImageInfo(!useCache || desc.textureRenderHint != Image.TextureRenderHint.CLAMP);
                (new Image.LoaderThread(desc.id, useCache, (Image.TexturedImageInfo)image$imageinfo)).start();
            }

            if (useCache)
            {
                inMemoryCache.put(desc, image$imageinfo);
            }
        }
    ...
    private static void loadTexture(String path, Image.TexturedImageInfo image, InputStream is) throws Exception
    {
        if (is instanceof BufferedInputStream)
        {
            loadTexture(path, image, (BufferedInputStream)is);
        }
        else
        {
            loadTexture(path, image, new BufferedInputStream(is));
        }
    }

    private static void loadTexture(String path, Image.TexturedImageInfo image, BufferedInputStream is) throws Exception
    {
        try
        {
            ...
            if (isGif(is))
            {
                ...
                BufferedImage bufferedimage3 = gifdecoder.getImage();
                image.width = bufferedimage3.getWidth();
                image.height = bufferedimage3.getHeight();
                ...
                image.tex = new Image.AnimatedTexture(new Image.GifAnimationSource(gifdecoder), image.ownTexture);
                image.loadingState = Image.LoadState.READY;
                Texteria.log.info("Loaded gif " + path + " with " + gifdecoder.getFrameCount() + " frames");
            }
            else if (isPng(is))
            {
                ...
                image.loadingState = Image.LoadState.READY;
                Texteria.log.info("Loaded png image " + path);
            }
            else
            {
                ...
                {
                    throw new Exception(path + " is not an image");
                }
                ...
                image.loadingState = Image.LoadState.READY;
                Texteria.log.info("Loaded image " + path);
            }

            Texteria.printDebug("Image " + path + " prepared in {} ns", new Object[] {Long.valueOf(System.nanoTime() - i)});
        }
        finally
        {
            is.close();
    ...
    private static void packSameSpritesDumb(List<Image.Sprite> sprites)
    {
        ...
        for (Image.Sprite image$sprite : sprites)
        {
            ...
            System.out.println(image$sprite.x + " " + image$sprite.y);
        }
    ...
    public static class AnimatedFrame implements Cloneable
    {
        public Image.Sprite sprite;
        public int delay;
        public int delayBefore;

        public AnimatedFrame(Image.Sprite sprite, int delay, int delayBefore)
        {
            this.sprite = sprite;
            this.delay = delay;
    ...
    public static class AnimatedTexture implements Image.Texture
    {
        ...
        public AnimatedTexture(Image.AnimationSource source, boolean ownTexture)
        {
            ...
            List<Image.Sprite> list = ownTexture ? new LinkedList() : null;
            ...
            for (int i = 0; i < this.frames.length - 1; ++i)
            {
                ...
                if (ownTexture)
                {
                    image$sprite = new Image.Sprite((Image.TextureAtlas)null, 0, 0, this.width, this.height);
                    list.add(image$sprite);
                }
                else
                {
                ...
                this.frames[i] = new Image.AnimatedFrame(image$sprite, source.getDelay(i), this.duration);
            ...
            this.frames[this.frames.length - 1] = new Image.AnimatedFrame(this.frames[this.frames.length - 2].sprite, 0, this.duration);
        }
        ...
        public Image.AnimatedFrame getCurrentFrame(Image.AnimationContext context, long time)
        {
            ...
            else
            {
                ...
                i = i - (long)(context.loopNum * this.duration);
    ...
    public static class BlocksTextureImageInfo extends Image.ImageInfo
    {
        ...
        public void render(Image element, long time)
        {
            ...
            GlStateManager.bindTexture(texturemap.b());
    ...
    private static class ByteLoaderThread extends Thread
    {
        ...
        ByteLoaderThread(String id, byte[] data, Image.TexturedImageInfo image)
        {
            super("Image loader");
            this.setDaemon(true);
            this.image = image;
        ...
        public void run()
        {
            try
            {
                Image.loadTexture(this.id, this.image, (InputStream)(new ByteArrayInputStream(this.data)));
            }
            catch (Exception exception)
            {
                Texteria.log.warn((String)("Loading image \'" + this.id + "\'."), (Throwable)exception);
    ...
    public static class GifAnimationSource implements Image.AnimationSource
    {
        ...
        public GifAnimationSource(GifDecoder decoder)
        {
        ...
        {
            return ((DataBufferInt)this.decoder.getFrame(frame).getRaster().getDataBuffer()).getData();
        }
        ...
        public int getWidth()
        {
            return this.decoder.getImage().getWidth();
        }

        public int getHeight()
        {
            return this.decoder.getImage().getHeight();
        }
    }
    ...
    private static class LoaderThread extends Thread
    {
        ...
        LoaderThread(String path, boolean useCache, Image.TexturedImageInfo image)
        {
            super("Image loader");
            this.setDaemon(true);
            this.path = path;
        ...
        public void run()
        {
            try
            {
                ...
                {
                    Image.loadTexture(this.path, this.image, (InputStream)(new FileInputStream(new File(Minecraft.getMinecraft().mcDataDir, this.path.substring(5)))));
                }
                else if (this.path.startsWith("#"))
                {
                    TexteriaResource texteriaresource = Texteria.instance.resourceManager.getResource(this.path.substring(1));

                    if (texteriaresource == null)
                    {
                        Texteria.log.warn("Resource " + this.path + " not exists");
                        this.image.loadingState = Image.LoadState.ERROR;
                        return;
                    }
                    ...
                    try
                    {
                        Image.loadTexture(this.path, this.image, texteriaresource.getInputStream());
                    }
                ...
                {
                    String s1 = DigestUtils.md5Hex(this.path.getBytes());
                    ...
                    if (!vimestorage$entry.exists())
                    {
                        try
                        {
                            ...
                            try
                            {
                                IOUtils.copy((new URL(this.path)).openStream(), outputstream);
                            }
                    ...
                    try
                    {
                        Image.loadTexture(this.path, this.image, (InputStream)vimestorage$entry.getInputStream());
                    }
                    catch (Exception exception)
                    {
                        vimestorage$entry.delete();
                ...
                {
                    Image.loadTexture(this.path, this.image, (new URL(this.path)).openStream());
                }
            }
